...
>
>  这个类的目的是为了高效地在CPU和GPU之间传输数据，同时避免不必要的复制操作。提供了对数据的同步访问，确保在读取数据时，数据是在正确的设备上初始化的
...
* **SyncedMemory**：管理主机（CPU）和设备（GPU）之间的内存分配和同步。
...
void check_device(); // 检查当前设备是否与数据所在的设备匹配
{
    ...
    CHECK(device == device_) << "device mismatch!";
    if (gpu_ptr_ && ouw_gpu_data_)
    {
...
* **to**：根据当前的同步状态(head_)执行操作
  * **UNINTIALIZED**：未初始化，申请内存
  * **HEAD_AT_CPU or GPU**：同步数据
